...
/* 匹配关键字 */
int seekkey(char *fname, char *keyword, char *nologfile, char *yeslogfile)
{
	char command[1024] = {};
	char comgrep[100] = {};
	char name[100] = {};
	...
	if (0 == size)
	{
		...
		/* 写入日志信息有 文件名，关键字 */
		strcat (msg, "关键字不匹配");
		my_syslog(nologfile, logname, msg);
		return -1;
#endif
	}
	else if (size > 0)
	{
		...
		/* 写入日志信息有 文件名，关键字 */
		strcat (msg, "关键字匹配");
		my_syslog(yeslogfile, logname, msg);
		return 0;
	}
}
...
int main()
{
	int res = seekkey ("a.txt", "tang", "bupipei.log", "pipei.log");
}
